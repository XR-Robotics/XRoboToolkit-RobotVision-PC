...
void NetworkVideoSource::start(const std::string& ip, int port, std::function<void(const char*, int, int, int)> frameCallback)
{
    std::cout << "NetworkVideoSource::start() called with ip: " << ip << " port: " << port << std::endl;
    // Create decoder
    m_decoder = new H264Decoder([frameCallback](const uint8_t* data, size_t size, int width, int height) {
        OutputDebugStringA((std::string("frameCallback decode output size: ") + std::to_string(size)).c_str());
    ...
    auto dataCallback = [this](const char* data, size_t length) {
        OutputDebugStringA((std::string("dataCallback decode input size: ") + std::to_string(length)).c_str());
...
void NetworkVideoSource::stop()
{
    ...
    if (m_networkThread.joinable()) {
        std::cout << "NetworkVideoSource: waiting for network thread to join" << std::endl;
